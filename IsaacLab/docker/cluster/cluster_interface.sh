...
submit_job() {
    ...
    ssh $CLUSTER_LOGIN "cd $CLUSTER_ISAACLAB_DIR && module load singularity && bash $CLUSTER_ISAACLAB_DIR/docker/cluster/$job_script_file \"$CLUSTER_ISAACLAB_DIR\" \"isaac-lab-$profile\" ${@}"
}
